# Lift Sources — DesignOps-plugin → Figmint
...
**Target repo (this one):** `c:\Users\jbabc\Documents\GitHub\Figmint\`
...
> **The legacy repo is a Claude Code / Cursor *skill pack* — Markdown orchestration + committed Plugin API JavaScript bundles. It is NOT a shipped Figma UI plugin.** ~80% of Figmint's deterministic logic can be ported by stripping MCP transport wrappers off the existing JS. The other ~20% (variable creation, REST calls) lives as agent-instruction prose in `phases/*.md` files and must be translated to TS, not copy-pasted.
...
|---|---|---|
| `README.md` | Designer-facing product README: eight slash commands, token architecture (5 collections), workflow, handoff template, plugin tree layout, maintainer commands (`sync-cache`, `bundle-component`, `build:props`). Documents MCP-inline policy + Org-tier REST requirement. | **Reference only.** Sunset PR (Sprint 11) rewrites this as a "Figmint companion" doc. |
| `CLAUDE.md` | Claude Code bootstrap: always load `memory.md` + `AGENTS.md`; lazy-load skills; `/create-component` Step 6 = five sequential `canvas-bundle-runner` Tasks. | **Reference only.** Sunset rewrites to point at Figmint. |
...
| `memory.md` | Token-dense workflow index: authority stack, end-to-end flow, session choreography, skills-at-a-glance, lazy-load map, maintainer npm commands. | **Reference.** Sunset replaces this with a thin Figmint companion summary. |
| `docs/PRD-figma-plugin.md` | Earlier draft PRD for the replacement native plugin (then called `DesignOps-figma-plugin`). Same mission/contracts/phasing/sunset as Figmint. | **Superseded by** `Figmint/Docs/PRD.md`. Treat the new repo's PRD as source of truth. |

### MCP anti-spiral concepts that disappear in Figmint
...
All hard-deleted by Sprint 11 (WO-051).

---

## 2. Skill inventory (9 skills) + Figmint disposition
...
| # | Skill | `SKILL.md` (legacy) | One-line purpose | Figmint §17 disposition |
|---|---|---|---|---|
...
| 7 | **accessibility-check** | `skills/accessibility-check/SKILL.md` | WCAG 2.1 AA + iOS/Android scale clones | **Deprecated** → Figma Agent pointer (Sprint 11, WO-054) |
| 8 | **new-language** | `skills/new-language/SKILL.md` | Duplicate frame → locale page; inline translation | **Deprecated** → Figma Agent pointer (Sprint 11, WO-054) |
| 9 | **canvas-bundle-runner** | `skills/canvas-bundle-runner/SKILL.md` | Run one committed bundle or assembled cc-* file | **Hard-deleted** (Sprint 11, WO-051) |
...
| Concern | Lives in | Figmint target |
|---|---|---|
...
| Source (legacy) | Lines | Bytes | Min bytes | Figmint target (Sprint 3) |
|---|---|---|---|---|
...
| Archetype | Source (legacy) | Lines | Bytes | Figmint target |
|---|---|---|---|---|
...
**Pipeline bundles** (5-call Step 6 sequence): `scaffold.mcp.js`, `properties.mcp.js`, `matrix.mcp.js`, `usage.mcp.js`, plus `cc-*` assembled files in `scripts/test-fixtures/.step6-orchestration-out/`. The 5-call sequence is an MCP-payload-budget artifact — Figmint executes the full pipeline in one Plugin API call.
...
Each markdown convention shard becomes a TypeScript helper in Figmint (per breakdown plan **Table B**). The rule moves from prose-the-LLM-must-remember to executable code.

### `skills/create-design-system/conventions/`

| Shard | Rule | Figmint target |
|---|---|---|
...
|---|---|---|
| `skills/create-component/registry.schema.json` (~33 lines JSON Schema) | Required: `fileKey`, `components` (map of name → `{ nodeId, key, pageName, publishedAt, version, optional cvaHash, composedChildVersions }`) | `packages/contracts/src/registry.v1.ts` (also defines on-disk `.figmint-registry.json` shape) |
...
| `qa-assemble-component-code.mjs` | **PORT** patterns → Figmint tests | |
...
| File | Tag | Notes |
|---|---|---|
| `agent-handoff.md` | **REPLACE** | Evolves to Figmint contract + plugin I/O; fields `active_file_key`, `token_css_path`, `open_items` |
| `workflow.md` | **KEEP-REFERENCE** | Agent-side workflow prose; Figmint has its own |
...
2. `Figmint/Docs/PRD.md` — §6 (functional reqs), §8 (data contracts), §17 (sunset).
3. `Figmint/Docs/lift-sources.md` (this file) — §0 corrections + your domain section.
...
| Legacy artifact | Figmint disposition |
|---|---|
...
| `/sync-design-system` | Thin agent skill consuming `drift-report.v1` (Sprint 11 WO-052) |
| `/dev-handoff` | Thin agent skill consuming `handoff-context.v1` (Sprint 11 WO-052) |
| `/code-connect` | Plugin PR emission + CI publish (Sprint 8) |
| `/accessibility-check`, `/new-language` | Deprecated; redirect to Figma Agent (Sprint 11 WO-054) |
| `/canvas-bundle-runner` + payload scripts + `.min.mcp.js` + `sync-cache.sh` + `measure-sigma.mjs` | Hard-deleted (Sprint 11 WO-051) |
| `AGENTS.md` MCP anti-spiral body + `conventions/16-mcp-use-figma-workflow.md` + `conventions/17-table-redraw-runbook.md` | Hard-deleted (Sprint 11 WO-051) |
...
- **Project board:** [Figmint Project #9](https://github.com/users/JBabcock-DL/projects/9)
