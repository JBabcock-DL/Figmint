...
Closing BUG-S5-004 unblocks `/vqa` Ship on WO-027 and brings Figmint's forward scaffold output to canvas-parity with the Foundations reference implementation in one pass instead of five.
...
- **OQ-5** (RESOLVED by D2) — Keep Figmint `_Doc/*` underscore prefix; do not rename to legacy `Doc/*`.
...
3. **Text-style names keep underscore prefix**: `_Doc/Section`, `_Doc/TokenName`, `_Doc/Code`, `_Doc/Caption`. Figmint convention from WO-012.
