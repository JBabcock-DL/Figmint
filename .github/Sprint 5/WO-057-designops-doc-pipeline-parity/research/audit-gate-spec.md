...
export async function runDocPipelinePreflightAudit(): Promise<AuditReportV1> {
  ...
  const typoVars = typoCollection
    ? (await figma.variables.getLocalVariablesAsync()).filter(
        (v) => v.variableCollectionId === typoCollection.id,
      )
    : [];
  ...
  const missingTokens = requiredColorTokens.filter(
    (name) => !themeVars.find((v) => v.name === name),
  );
  ...
  const missingFontFamilyVars = requiredFontFamilyVars.filter(
    (name) => !typoVars.find((v) => v.name === name),
  );

  const textStyles = await figma.getLocalTextStylesAsync();
  const requiredTextStyles = ['_Doc/Section', '_Doc/TokenName', '_Doc/Code', '_Doc/Caption'];
  const missingTextStyles = requiredTextStyles.filter(
    (name) => !textStyles.find((s) => s.name === name),
  );
