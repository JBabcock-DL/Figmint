...
- [ ] **Step 5** — Author `.github/workflows/ci.yml` using the paste-ready GHA recipe in research §`.github/workflows/ci.yml skeleton`:
  ...
  - Step ordering (fail-fast, cheapest → most expensive, **lint + format kept as two CI steps for clearer failure attribution**):
    ...
    8. `npm run build:org`
- [ ] **Step 6** — Add a CI status badge to `README.md` at the top of the file (immediately under the H1): `![CI](https://github.com/JBabcock-DL/Figmint/actions/workflows/ci.yml/badge.svg)`. If `README.md` does not yet exist or is empty, create/seed it with an H1 (`# Figmint`) + the badge line + a one-line description; WO-002's scaffold should produce one, but verify and add the badge regardless.
...
- **OQ-1 (from research §Open Questions §2):** Accept `typescript-eslint` `strict-type-checked` semver churn? **Default: YES** (locked by research recommendation — Figmint is in early dev, every new bug rule pays dividends; flagged here for visibility only). If a minor `typescript-eslint` upgrade introduces churn-level rule changes during Sprint 2+, revisit by pinning a narrower range or dropping to `recommended-type-checked`.
- **OQ-2 (from research §Open Questions §4):** Ban default exports project-wide via `import-x/no-default-export`? **Default: NO** (less aggressive — Figmint's source surface is too small to be opinionated yet). Flagged as a Sprint 2+ future tightening; reconsider when `packages/contracts/` has multi-file barrel exports.
