...
| 3   | Final shape as single TS type in `tokens.v1.ts`; JSON Schema regenerates | **PASS** | Full `TokensV1` tree in `packages/contracts/src/tokens.v1.ts`; `npm run build -w @detroitlabs/figmint-contracts` green; `dist/tokens.v1.schema.json` regenerated                |
...
```
npm run build -w @detroitlabs/figmint-contracts  → exit 0
