...
| Sprint 2 ships JSON serialization of `AuditReportV1` only (GFM deferred to WO-019)                               | **PASS** | Contract in `packages/contracts/src/auditReport.v1.ts` (`v: 1`, `kind: 'audit-report'`); exported from `@detroitlabs/figmint-contracts`; JSON Schema at `packages/contracts/dist/audit-report.v1.schema.json`. No GFM formatter shipped. |
