...
- [x] Step 8 — **`codeSyntax.ts` — `mapCodeSyntax`:** Start from shallow copy of `token.codeSyntax` (treat `''` as absent). If `token.collection === 'theme'`, return stored partial only. Else for each platform in `['WEB','ANDROID','iOS']`, use stored value when present, else `deriveCodeSyntax(token, platform)`. Omit keys whose value is absent/empty. Export types from `@detroitlabs/figmint-contracts`.
...
| Dependency                     | Role                                                                                                                                                                                                                                   |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **WO-055**                     | `TokensV1` / `CanonicalToken` + `CodeSyntaxPlatform` on `@detroitlabs/figmint-contracts` — **required** for production types; Phase 1 may use fixture objects matching research sketch until WO-055 merges.                            |
...
| 3   | Partial DTCG `$extensions.figmint.codeSyntax` on Primitives?       | **Yes** — hybrid: stored platform wins, derive the rest.                                                                                    |
