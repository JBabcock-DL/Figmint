...
> **Status:** `/research` complete (2026-05-28). Sprint 6 is **blocked on WO-058 Phase 1** (snapshot + GitHub-Desktop I/O) before `/build` on WO-029–032.
...
| **WO-026** (#29, Sprint 5) | **Superseded by WO-058** | `.figmint-registry.json` deleted; registry SSOT = canvas pluginData snapshot |

**Build order:** WO-058 Phase 1 (snapshot + `figmint.json` + Settings card) → WO-029 ∥ WO-030 → WO-031 → WO-032 → badge follow-up (was WO-033 tail).
...
1. **WO-058 Phase 1** — snapshot store, delete repo registry file, `figmint.json`, Settings Fetch/Pull/Push card
...
| # | Decision | Owner |
| - | -------- | ----- |
| 1 | Snapshot SSOT = hidden node pluginData on Figmint Output page (`figmint:snapshot:v1`) | WO-058 / WO-028 |
| 2 | Registry SSOT = same snapshot envelope (delete `.figmint-registry.json`) | WO-058 |
...
| -- | -------- | ----- | ------ |
| OQ-S6-1 | Snapshot envelope: single JSON blob vs sharded pluginData keys per domain | WO-058 `/plan` | **Default: single `figmint:snapshot:v1` blob** — see WO-028 research |
| OQ-S6-2 | Component repo side: read specs from `figmint.json` paths vs registry map in snapshot | WO-058 + WO-030 | **Default: snapshot registry map + lazy spec fetch by path** |
