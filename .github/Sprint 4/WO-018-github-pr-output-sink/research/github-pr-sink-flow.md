...
3. Create head branch `fighub/{contractKind}-{date}` (suffix on collision).
...
### 3. Branch naming: `fighub/{contractKind}-{date}`

Default pattern (configurable via Settings / export sheet):

```
fighub/{contractKind}-{yyyy-MM-dd}
```
...
- Base directory from connected-repo settings (default example: `docs/fighub/`).
...
```
docs/fighub/drift-report-2026-05-27.v1.json
docs/fighub/drift-report-2026-05-27.v1.md
```
...
**Title:** Designer commit message (first line) or auto: `fighub: {contractKind} export ({date})`.
...
_Generated by FigHub v{pluginVersion}_
...
6. **Integration test:** fixture repo (or `@octokit/fixtures` pattern) against a test org repo — gated behind env var `FIGHUB_GITHUB_TEST_TOKEN` in CI skip by default.
...
POST /repos/{owner}/{repo}/git/trees
{
  "base_tree": "def456",
  "tree": [
    {"path":"docs/fighub/drift-report-2026-05-27.v1.json","mode":"100644","type":"blob","sha":"blobSha1"},
    {"path":"docs/fighub/drift-report-2026-05-27.v1.md","mode":"100644","type":"blob","sha":"blobSha2"}
  ]
}
```
