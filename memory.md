...
- **Current sprint folder:** `.github/Sprint 4/` — **WO-016–020 In Research**; **WO-021 deferred** to Context Backlog (2026-05-27). **Single build** — `npm run build`, root `manifest.json` (GitHub network enabled), `src/config/flags.ts` (all features `true`). Dual Community/Org gating → WO-021 when re-prioritized. Plan order: **WO-019 → WO-017 → WO-016 → WO-018 → WO-020** (WO-021 out).
...
- **Single build (2026-05-27):** Community vs Org dual manifests deferred — WO-021 on Context Backlog. Use `npm run build` + root `manifest.json` + `src/config/flags.ts` (all flags `true`). Do not re-split builds without reopening WO-021. Legacy manifests: `manifest.community.json`, `manifest.org.json` (reference only).
- **Don't move Sprint tickets to In Planning after summary-only `/research`.** Research must satisfy **research-quality-bar.md**: repo inventory, official API facts, decision log, pre-plan spikes, risk register. WO-006 `io-subsystem-design.md` (~500 lines) is the depth bar for platform-constraint tickets; multi-integration tickets (OAuth + REST) need 250+ lines minimum. Sprint 4 index: `.github/Sprint 4/research/sprint-4-io-gating-research-index.md`.
...
- _2026-05-27 — **Sprint 4 research complete.** WO-016–021 moved to **In Research** on Project #9; six research docs under `.github/Sprint 4/*/research/`. Locked: Device Flow OAuth (WO-016), Sink interface + thread split (WO-017), Git Data API PR sink (WO-018), dual-format serializers (WO-019), ExportSheet UI (WO-020), flags integration (WO-021 after 016–020).
