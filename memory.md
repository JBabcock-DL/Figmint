...
- **Don't use `performance.now()` in the plugin main thread.** Figma's sandbox (QuickJS-based) does not expose the `performance` global — `pushPrimitives.ts` throws `'performance' is not defined` the moment the Push handler runs. Use `Date.now()` instead; 1 ms granularity is fine for our BenchRecord (smallest phase slice rounds to 0 or 1 ms, itself a meaningful signal). The UI iframe is a real browser and DOES have `performance.now()` — only the main thread is constrained. Symmetric to the `?.`/`replaceAll`/`??` constraint also documented above.
